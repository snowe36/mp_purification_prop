...
def gather() -> dict[str, list[Record]]:
    ...
    gfp_rows = gfp.load()
    print("loading gpcrdb ...", flush=True)
    gpcr_rows = gpcrdb.load()
    print("loading detergent screens ...", flush=True)
    screen_rows = screens.load()
    return {
        "curnow": labelled,
...
def run_census(
    ...
    *,
    ...
    gpcr_rows = bags.get("gpcrdb") or []
    gpcr_sum = gpcrdb.summarize(gpcr_rows) if gpcr_rows else {
        "n_rows": 0,
        "n_solubilization": 0,
        "n_pdb": 0,
        "detergents": {},
    ...
    screen_rows = bags.get("screens") or []
    screen_sum = screens.screen_counts(screen_rows)
    ...
    bcond_prior = int(gpcr_sum.get("n_pdb") or 0) >= STOP["Bcond_min_tm"]
    ...
    result = {
        ...
        "gpcrdb": {
            **gpcr_sum,
            "claim_B_conditions_prior": bcond_prior,
        },
        "screens": screen_sum,
        "structure": {
